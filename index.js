...
async function handleRequest(request) {
    ...
    //return new Response(url.pathname +" 1")



    if( pathname.includes("download.php?") ){//如果是下载链接
        //echo "开始下载<br>";
...
async function GetVideoInfo(){
    ...
    else if( stristr($videoId,"bv") ){//检测是否是bv类视频
        $videoIdInfo["type"] = 2;
        $videoIdInfo["id"] = stristr($videoId,"bv");
    }
    else if( stristr($videoId,"ss") ){//检测是否是ss类视频
        $videoIdInfo["type"] = 3;
        $videoIdInfo["id"] = stristr($videoId,"ss");
    }
    else if( stristr($videoId,"ep") ){//检测是否是ep类视频
        $videoIdInfo["type"] = 4;
...
function stristr (firststr, secondstr, bool) {
  let pos = 0
  firststr += ''
  pos = firststr.toLowerCase()
    .indexOf((secondstr + '')
      .toLowerCase())
  if (pos === -1) {
    return false
